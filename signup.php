...
if (!empty($_POST)) {
    ...
    $regexPassword = '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{12,}$/'; // generated by the AI 
    ...
    elseif (getUserByEmail($email)) {
        ...
        // Insertion of the user in the database : -> he signed up
        $id = insertUserIntoDb($user, $passwordHashed);

        if ($id) {
            $_SESSION['user_id'] = $id;
            $_SESSION['username'] = $username;
            
            // Redirection at the page login :
